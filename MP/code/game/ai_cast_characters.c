...
int AIChar_GetPainLocation( gentity_t *ent, vec3_t point ) {
	...
	if ( !trap_GetTag( ent, NULL, painTagNames[0], &or ) ) { //if ( !trap_GetTag( ent->s.number, NULL, painTagNames[0], &or ) ) {
		return 0;
	}
	...
	for ( tagIndex = 0, bestDist = 0, bestTag = -1; painTagNames[tagIndex]; tagIndex++ ) {
		...
		if ( trap_GetTag( ent, NULL, painTagNames[tagIndex], &or ) ) { //if ( trap_GetTag( ent->s.number, NULL, painTagNames[tagIndex], &or ) ) {
