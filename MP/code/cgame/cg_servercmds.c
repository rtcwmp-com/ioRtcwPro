...
void CG_ParseWolfinfo( void ) {
	...
	// RtcwPro Announce game in progress if we are really playing
...
static void CG_MapRestart( void ) {
	...
	cg.popinPrintTime = 0;	// RtcwPro - reset pop in prints..
...
//	CG_ClearTrails();
	...
	cg.mapRestart = qtrue;
	cgs.dumpStatsTime = 0;	// RtcwPro Stats
	...
	cg.latchAutoActions = qfalse;			// RtcwPro - Auto Actions
...
*/
	// RtcwPro - Not in demo if disabled
...
void CG_dumpStats(qboolean endOfRound) {
	...
	const char *winner = CG_ConfigString(CS_MULTI_MAPWINNER);
	const char* buf = Info_ValueForKey(winner, "winner");
...
// L0 -End OSP Stats dump
	...
	if ( !strcmp( cmd, "chat" ) ) {
		...
		// RtcwPro - No chat if it's disabled..
